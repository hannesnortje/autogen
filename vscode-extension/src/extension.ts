...
export async function activate(context: vscode.ExtensionContext) {
    ...
    // Initialize enhanced status bar first
    statusBar = new AutoGenStatusBar(context, mcpClient, sessionTreeProvider);

    // Register status bar commands with the mcpClient and statusBar
...
async function startMcpServer(): Promise<boolean> {
    try {
        ...
        // Use configured server path or default to autogen directory
        const defaultServerPath = '/media/hannesn/storage/Code/autogen';
        const serverPath = config.get<string>('serverPath', defaultServerPath);

        // Verify the server path exists and has pyproject.toml
        try {
            const pyprojectPath = vscode.Uri.file(`${serverPath}/pyproject.toml`);
            await vscode.workspace.fs.stat(pyprojectPath);
        } catch (error) {
            const choice = await vscode.window.showErrorMessage(
                `AutoGen MCP server not found at: ${serverPath}\nMake sure the AutoGen project is available at this location.`,
                'Select Server Path',
                'Cancel'
            );

            if (choice === 'Select Server Path') {
                const selected = await vscode.window.showOpenDialog({
                    canSelectFolders: true,
                    canSelectFiles: false,
                    canSelectMany: false,
                    title: 'Select AutoGen Project Directory'
                });

                if (selected && selected[0]) {
                    // Update configuration with selected path
                    await config.update('serverPath', selected[0].fsPath, vscode.ConfigurationTarget.Global);
                    return startMcpServer(); // Retry with new path
                }
            }
        ...
        const serverCommand = config.get<string>('serverCommand', 'poetry run python -m src.autogen_mcp.mcp_server');
